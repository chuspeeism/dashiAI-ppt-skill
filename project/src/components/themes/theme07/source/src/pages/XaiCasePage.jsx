/**
 * XaiCasePage — P64 实时数据生态 (xAI Case · Image-led · Case Card)
 *
 * A single-company "case card": one representative company profiled with a
 * tall hero image, a slanted brand-style name, an ecosystem tag cluster and a
 * strip of headline metrics (one highlighted). Part of the case-card series
 * (OpenAI / Anthropic / xAI) — the three share the `.aic-cc` template scope and
 * an identical prop contract; only the authored text differs.
 *
 ...
 * is prop-driven (0–2); at 0 the hero falls back to the brand lens graphic and
 * the composition stays intact.
 *
...
const COPY = {
  ...
  enName: 'XAI',
  badge: '实时数据',
  lead: 'xAI 以实时数据与马斯克生态作为差异化；核心资产不是模型本身，而是实时数据与生态流量。',
  ...
  metrics: [
    { label: '单笔融资', value: '50', unit: '亿美元', num: true },
    { label: '公司估值', value: '500', unit: '亿美元', num: true },
    { label: '数据入口', value: 'X 平台', num: false },
    { label: '生态协同', value: '多模态', num: false },
  ],
  // ecosystem tags (order fixed; count is prop-driven)
  tags: ['实时数据', '实时搜索', '生态流量'],
};
...
export const defaultProps = {
  ...
  showTags: true,          // ecosystem tag cluster
...
export const controls = [
  { key: 'eyebrow', label: '眉标', type: 'text', default: 'xAI Case' },
  { key: 'title', label: '标题', type: 'text', default: '实时数据生态' },
  { key: 'titleTail', label: '副标题', type: 'text', default: 'xAI 案例页' },
  { key: 'logic', label: 'logic', type: 'text', default: '实时数据逻辑' },
  { key: 'company', label: 'company', type: 'text', default: 'xAI' },
  { key: 'enName', label: 'enName', type: 'text', default: 'XAI' },
  { key: 'badge', label: 'badge', type: 'text', default: '实时数据' },
  { key: 'lead', label: '导言', type: 'text', default: 'xAI 以实时数据与马斯克生态作为差异化；核心资产不是模型本身，而是实时数据与生态流量。' },
  { key: 'closing', label: '结语', type: 'text', default: '独特数据入口可以成为模型差异化。' },
  ...
  { key: 'metricCount', label: '卡片数量', type: 'slider', default: 3, min: 2, max: 4, step: 1,
  ...
  { key: 'showTags', label: '标签文案', type: 'toggle', default: true,
    description: '生态标签行的显隐。' },
  { key: 'tagCount', label: '标签数量', type: 'slider', default: 3, min: 2, max: 3, step: 1,
    description: '生态标签数量（2–3）。', showWhen: (p) => p.showTags },
...
export default function XaiCasePage(props) {
